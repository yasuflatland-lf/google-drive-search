...
portletURL.setParameter("searchRepositoryId", String.valueOf(searchRepositoryId));
		...
		<aui:form action="<%= searchURL %>" method="get" name="fm">
			...
			try {
				...
				for (int i = 0; i < searchResults.size(); i++) {
					...
					else if (className.equals(GoogleDriveModel.class.getName())) {
						googleDocStat = true;
					}
			%>

					<c:choose>
						<c:when test="<%= (googleDocStat == true) %>">
							<%
							Document document = hits.toList().get(i);
							String thumbnailSrc = "thumbnailSrc";
							%>
							<liferay-ui:app-view-search-entry
								...
								rowCheckerId="<%= String.valueOf(searchResult.getClassPK()) %>"
								rowCheckerName="<%= GoogleDriveModel.class.getSimpleName() %>"
								showCheckbox="<%= false %>"
								status="<%= 0 %>"
								thumbnailSrc="<%= document.get(thumbnailSrc) %>"
								title="<%= document.get(Field.TITLE) %>"
								url="<%= document.get(Field.URL) %>"
							/>
